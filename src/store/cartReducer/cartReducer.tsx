import { CartActionTypes, ADD_TO_CART, REMOVE_FROM_CART, INCREASE_QUANTITY, DECREASE_QUANTITY, BUY_NOW_PRODUCT } from '../../types/CartActionType';
  ...
  const cartReducer = (state = initialState, action: CartActionTypes): CartState => {
    switch (action.type) {
      ...
      case REMOVE_FROM_CART:
        const removedItem = state.cart.find(item => item.id === action.payload.productId);
      ...
      case INCREASE_QUANTITY:
        const existingItemIncrease = state.cart.findIndex(item => item.id === action.payload.id);
      ...
      case DECREASE_QUANTITY:
        const existingItemDecrease = state.cart.findIndex(item => item.id === action.payload.id);
